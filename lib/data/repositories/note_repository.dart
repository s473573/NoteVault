...
class NoteRepository extends GetxController{
  ...
  Future<List<Note>> getAllNotes() async {
    try {
      ...
      if (storedNotes != null) {
        final List<Map<String, dynamic>> noteList = List<Map<String, dynamic>>.from(jsonDecode(storedNotes).map((note) => Map<String, dynamic>.from(note)));
  ...
  Future<void> saveNotes() async {
    try {
      final List<Map<String, dynamic>> noteList = _notes.map((note) => note.toJson()).toList();
